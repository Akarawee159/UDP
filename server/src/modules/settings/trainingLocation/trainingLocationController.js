...
const model = require('./trainingLocationModel');
...
function parsePayload(body) {
  ...
  const G_NAME_EN = String(body.G_NAME_EN || '').trim();
  return { G_CODE, G_NAME, G_NAME_EN };
}

function ensureRequired({ G_CODE, G_NAME }) {
  const missing = [];
  if (!G_CODE) missing.push('G_CODE');
  if (!G_NAME) missing.push('G_NAME');
  if (missing.length) {
...
/** GET /api/religion/check-code?code=...&excludeId=... */
...
async function create(req, res, next) {
  try {
    ...
    if (io) io.emit('religion:upsert', row);
...
async function remove(req, res, next) {
  try {
    ...
    // broadcast realtime
    const io = req.app.get('io');
    if (io) io.emit('religion:delete', { G_ID });
