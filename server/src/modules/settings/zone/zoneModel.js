// src/modules/settings/zone/zoneModel.js
'use strict';
...
async function checkCodeDuplicate(G_CODE, excludeId = null) {
  let sql = `SELECT G_ID FROM tb_zone WHERE G_CODE = ?`;
  const params = [G_CODE];
  if (excludeId != null) {
    sql += ` AND G_ID <> ?`;
...
async function create({ G_CODE, G_NAME }) {
  ...
  try {
    ...
    const [lastRows] = await conn.query(
      'SELECT G_ID FROM tb_zone ORDER BY G_ID DESC LIMIT 1 FOR UPDATE'
    );

    const nextId = lastRows.length ? Number(lastRows[0].G_ID) + 1 : 1;

    const sql = `
      INSERT INTO tb_zone (G_ID, G_CODE, G_NAME)
      VALUES (?, ?, ?)
    `;
...
async function remove(G_ID) {
  const sql = `DELETE FROM tb_zone WHERE G_ID = ?`;
