...
function parsePayload(body) {
  ...
  // ✅ รับค่า branch_code
  const branch_code = String(body.branch_code || '').trim();
  return { G_CODE, G_NAME, branch_code };
}

function ensureRequired({ G_CODE, G_NAME, branch_code }) {
  const missing = [];
  if (!G_CODE) missing.push('กรุณาพิมพ์รหัส');
  if (!G_NAME) missing.push('กรุณาพิมพ์ชื่อ');
  if (!branch_code) missing.push('กรุณาเลือกสาขา');
  if (missing.length) {
...
/** GET /api/department/check-code?code=...&excludeId=... */
...
// ✅ เพิ่มฟังก์ชัน getBranchCodes
async function getBranchCodes(_req, res, next) {
  try {
    const rows = await model.listBranchCodes();
    res.json({ success: true, data: rows });
  } catch (err) {
    next(err);
  }
...
async function create(req, res, next) {
  try {
    ...
    if (io) io.emit('department:upsert', row);
...
async function remove(req, res, next) {
  try {
    ...
    // broadcast realtime
    const io = req.app.get('io');
    if (io) io.emit('department:delete', { G_ID });
