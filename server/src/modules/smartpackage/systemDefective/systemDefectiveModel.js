...
async function generateRefID(draft_id, user_id) {
  ...
  const prefix = `DF${dateStr}`; // Prefix ที่ต้องการค้นหา เช่น RF040226

  // ค้นหา RefID ล่าสุดของวันนี้ ที่ขึ้นต้นด้วย DF เท่านั้น (เพื่อไม่ให้นับรวมเอกสารประเภทอื่น)
...
async function updateBookingHeader(draft_id, body, user_id) {
  ...
  // ดึงข้อมูลเดิมมาเช็คก่อนว่าสถานะปัจจุบันคืออะไร
  const [bookingRes] = await db.query(`SELECT refID, is_status FROM booking_asset_lists WHERE draft_id = ?`, [draft_id]);
  let currentRefID = bookingRes[0]?.refID;
  const currentStatus = String(bookingRes[0]?.is_status);
  ...
  // 💡 ป้องกันไม่ให้ is_status โดนบังคับเป็น 141 ถ้ามันถูกปลดล็อคมา (144) 
  const newStatus = (currentStatus === '144') ? '144' : '141';

  await db.query(`
        UPDATE booking_asset_lists
        SET refID = ?, booking_remark = ?, origin = ?, destination = ?, is_status = ?, updated_by = ?, updated_at = ?
        WHERE draft_id = ?
    `, [currentRefID, booking_remark, origin, destination, newStatus, user_id, now, draft_id]);

  // ✅ อัปเดตตาราง tb_asset_lists ทั้ง origin, destination และ current_address อ้างอิงจาก draft_id
  await db.query(`
        ...
        SET asset_origin = ?, 
            ...
            updated_at = ? 
        WHERE draft_id = ?
    `, [origin, destination, destination, now, draft_id]);

  if (currentRefID) {
    // ✅ อัปเดต Detail สำหรับของที่มีประวัติชำรุดไปแล้วเพื่อให้ข้อมูลสอดคล้องกัน
    await db.query(`
        ...
        SET asset_origin = ?, 
            ...
            current_address = ?,
            updated_at = ? 
        WHERE refID = ? AND asset_status = '147' AND is_status = '145' 
    `, [origin, destination, destination, now, currentRefID]);
  }
...
async function getAssetsDetailByRefID(refID) {
  const sql = `
        ...
        WHERE d.refID = ? AND d.is_status = '145' AND d.asset_status = '147'
...
async function getAllBookings(startDate, endDate) {
  const sql = `
    SELECT 
        ...
        (
          CASE 
            -- 1. กรณีแจ้งชำรุดเรียบร้อย (145) ให้นับจำนวนจากตารางประวัติ (Detail)
            WHEN b.is_status = '145' THEN 
              (
                SELECT COUNT(*)
                FROM tb_asset_lists_detail d1
                WHERE d1.refID = b.refID 
                AND d1.is_status = '145' 
                AND d1.asset_status = 147
                AND d1.scan_at = (         
            ...
            -- 2. กรณีปลดล็อคแก้ไข (144) ให้นับจำนวนจากตารางหลัก (Master) ที่สถานะเป็น 147
            WHEN b.is_status = '144' THEN 
              (
                SELECT COUNT(*) 
                FROM tb_asset_lists a 
            ...
            -- 3. กรณีใบเบิกใหม่/อยู่ในตะกร้า (140, 141, 146) ให้นับของในตะกร้าซึ่งสถานะจะเป็น 148
            ELSE 
              (SELECT COUNT(*) FROM tb_asset_lists a WHERE a.draft_id = b.draft_id AND a.asset_status = 148)
          END
        ) as attendees,
...
// 2. เพิ่มฟังก์ชันสำหรับสถานะ 146 (แก้ไขข้อมูลแจ้งชำรุด)
async function editHeaderBooking(draft_id, user_id) {
  const now = getThaiNow();
  await db.query(`
        UPDATE booking_asset_lists SET is_status = '146', updated_by = ?, updated_at = ? WHERE draft_id = ?
    `, [user_id, now, draft_id]);
  return true;
}

// 3. ปรับฟังก์ชัน Finalize (แจ้งชำรุดสมบูรณ์ - รวบ 141->145 และ 144->145 ไว้ด้วยกัน)
async function finalizeBooking(draft_id, user_id, headerData = {}) {
  ...
  const sqlInsertDetail = `
        ...
        WHERE t.draft_id = ? AND t.asset_status = 148
    `;
  ...
  // ✅ 3. อัปเดตตารางใบงาน (Header) ให้เป็น 145
  await db.query(`
        UPDATE booking_asset_lists 
        SET is_status = '145', updated_by = ?, updated_at = ? 
        WHERE draft_id = ?
    `, [user_id, now, draft_id]);
...
async function unlockBooking(draft_id, user_id) {
  const now = getThaiNow();
  // Change logic: Update to status '144' instead of '141'
  await db.query(`
        UPDATE booking_asset_lists
        SET is_status = '144',
            updated_by = ?,
...
async function scanCheckIn(uniqueKey, draft_id, refID, user_id) {
  ...
  if (!origin || !destination) {
    return { success: false, code: 'INVALID_BOOKING', message: 'ไม่พบข้อมูลต้นทางหรือปลายทางของใบแจ้งชำรุดนี้' };
  }

  // ดักจับกรณีสแกนไปแล้วหรือแจ้งชำรุดซ้ำ
  if (item.asset_status == 148) {
    if (item.refID === refID) {
      return { success: false, code: 'ALREADY_SCANNED', message: `สแกนกล่องซ้ำ`, data: item };
    } else {
      return { success: false, code: 'INVALID_STATUS', message: `กล่องนี้อยู่ระหว่างการแจ้งชำรุดในรายการอื่น`, data: item };
    }
  }

  // ตรวจสอบสถานะต้องเป็น 100 หรือ 101 เท่านั้น
  if (item.asset_status != 100 && item.asset_status != 101) {
    return { success: false, code: 'INVALID_STATUS', message: `กล่องนี้สถานะไม่พร้อมแจ้งชำรุด (ต้องเป็นสถานะ 100 หรือ 101 เท่านั้น ปัจจุบัน: ${item.asset_status})`, data: item };
  }

  // ตรวจสอบเส้นทาง: tb_asset_lists.current_address ต้องตรงกับ booking_asset_lists.origin
  if (item.current_address !== origin) {
    return { success: false, code: 'INVALID_ROUTING', message: `ที่อยู่ปัจจุบันของทรัพย์สิน (${item.current_address || 'ไม่ระบุ'}) ไม่ตรงกับ "แจ้งชำรุดจาก" (${origin})`, data: item };
  }
  ...
  const itemSnapshot = {
    ...
    is_status: '142',     // อัปเดตสถานะ is_status = 142
    asset_status: 148,    // อัปเดตสถานะ asset_status = 148
    asset_action: 'สแกนแจ้งชำรุด',
    scan_by: user_id,
    scan_at: now,
    updated_at: now,
    updated_by: user_id
...
async function returnSingleAsset(assetCode) {
  ...
  if (item.booking_status === '144') {
    // อัปเดตตารางหลัก 
    await db.query(`UPDATE tb_asset_lists SET asset_status = 147, is_status = '109', draft_id = NULL, refID = NULL, updated_at = ? WHERE asset_code = ?`, [now, assetCode]);

    // ✅ 1. อัปเดต Detail เดิมให้เป็น 109 (ป้องกันค้างดึงมาแสดงตอน 145)
    await db.query(`UPDATE tb_asset_lists_detail SET asset_status = 103, is_status = '109', updated_at = ? WHERE refID = ? AND asset_code = ?`, [now, item.refID, assetCode]);

    // ✅ 2. Insert Detail ประวัติใหม่ระบุ Action เป็น 'ยกเลิกแจ้งชำรุด'
    const itemSnapshot = {
      ...item,
      is_status: '109',
      asset_status: 147,
      asset_action: 'ยกเลิกแจ้งชำรุด',
      updated_at: now,
      scan_at: now
    };
    ...
    await db.query(`UPDATE tb_asset_lists SET asset_status = 147, draft_id = NULL, refID = NULL, updated_at = ? WHERE asset_code = ?`, [now, assetCode]);
  }
...
async function returnToStock(ids) {
  ...
  for (const assetCode of ids) {
    // 1. ดึงสถานะใบเบิกปัจจุบัน เพื่อดูว่าเป็น 141 หรือ 144
    const [rows] = await db.query(`
    ...
    if (prevData) {
      ...
      // 4. ถ้าเป็นการยกเลิกตอนสถานะ 144 (ปลดล็อค) 
      // 💡 เปลี่ยนจากการ DELETE เป็น UPDATE ประวัติ 103/145 เป็น 143/143
      if (bookingStatus === '144') {
        await db.query(`
                  UPDATE tb_asset_lists_detail 
                  SET asset_status = 143, 
                      is_status = '143',
                      asset_action = '(ปลดล็อค)ยกเลิกแจ้งชำรุด',
                      updated_at = ?
                  WHERE asset_code = ? 
                    AND asset_status = 147 
                    AND is_status = '145' 
                    AND refID = ? 
                  ORDER BY created_at DESC 
                  LIMIT 1
      ...
      // 5. ลบประวัติ 148/142 ใน Detail ทิ้ง (อันนี้ควรลบตามเดิม เพราะถือว่าแค่ถูกดึงลงตะกร้าแล้วดึงกลับ ยังไม่ได้มีผลทางบัญชี)
      await db.query(`
...
async function confirmOutput(draft_id, user_id) {
  ...
  const sqlInsertDetail = `
        ...
        SELECT
            ...
            t.asset_remark, t.asset_usedfor, t.asset_brand, t.asset_feature,
            t.asset_supplier_name, t.label_register, t.partCode, t.print_status,
            t.asset_status, 'ยืนยันแจ้งชำรุด', '145', 
...
// ดึงรายการที่ชำรุด (asset_status = 147)
async function getDefectiveItems() {
  const sql = `
      SELECT a.asset_code, a.asset_detail, a.refID, a.updated_at,
             s1.G_NAME as asset_status_name
      FROM tb_asset_lists a
      LEFT JOIN tb_erp_status s1 ON a.asset_status = s1.G_CODE AND s1.G_USE = 'A1'
      WHERE a.asset_status = 147
      ORDER BY a.updated_at DESC
  `;
...
// รับเข้าคลัง (อัปเดตสถานะ 100, is_status 105)
async function receiveToStock(assetCodes, userId) {
  if (!assetCodes || assetCodes.length === 0) return false;
  const now = getThaiNow();
  ...
  await db.query(`
      ...
      SET asset_status = 100, 
          ...
          updated_at = ? 
      WHERE asset_code IN (?)
  `, [userId, now, assetCodes]);

  // 2. บันทึก History ลง Detail เพื่อเก็บประวัติการเคลื่อไหว
  const sqlInsertDetail = `
      INSERT INTO tb_asset_lists_detail (
          draft_id, refID, asset_code, asset_detail, asset_type, asset_date,
          doc_no, asset_lot, asset_holder, asset_location,
          ...
          asset_status, asset_action, is_status,
          create_date, created_by, created_at, updated_by, updated_at, scan_by, scan_at,
          asset_origin, asset_destination, asset_model, asset_responsible_department, asset_source, current_address
      )
      SELECT
          draft_id, refID, asset_code, asset_detail, asset_type, asset_date,
          doc_no, asset_lot, asset_holder, asset_location,
          ...
          asset_remark, asset_usedfor, asset_brand, asset_feature,
          asset_supplier_name, label_register, partCode, print_status,
          100, 'รับเข้าคลัง (จากชำรุด)', '105',
          create_date, created_by, created_at, ?, ?, ?, ?,
          asset_origin, asset_destination, asset_model, asset_responsible_department, asset_source, current_address
      FROM tb_asset_lists
      WHERE asset_code IN (?)
  `;
  await db.query(sqlInsertDetail, [userId, now, userId, now, assetCodes]);
...
// ดึงรายการที่รอทำคืนชำรุด (asset_status = 149) สำหรับหน้า TabDefective2
async function getDefectiveCheckoutItems() {
  const sql = `
      SELECT a.asset_code, a.asset_detail, a.refID, a.updated_at,
             s1.G_NAME as asset_status_name
      FROM tb_asset_lists a
      LEFT JOIN tb_erp_status s1 ON a.asset_status = s1.G_CODE AND s1.G_USE = 'A1'
      WHERE a.asset_status = 149
      ORDER BY a.updated_at DESC
  `;
  const [rows] = await db.query(sql);
  return rows;
...
// แจ้งจ่ายออก (TabDefective) -> อัปเดตสถานะ 149, is_status 117
async function checkoutDefective(assetCodes, userId) {
  if (!assetCodes || assetCodes.length === 0) return false;
  const now = getThaiNow();
...
// ทำคืนชำรุด (TabDefective2) -> อัปเดตสถานะ 147, is_status 103
async function returnDefective(assetCodes, userId) {
  if (!assetCodes || assetCodes.length === 0) return false;
  const now = getThaiNow();
