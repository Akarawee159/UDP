...
import {
    ...
    SaveOutlined, DeleteOutlined, PushpinOutlined,
...
function ModalForm({ open, record, onClose, onSuccess, onDelete }) {
    ...
    const fetchDetail = useCallback(async (id) => {
        try {
            setFetching(true);
            // หมายเหตุ: ใช้ endpoint /area ตามโค้ดเดิม
    ...
    const validateCode = (_rule, value) => new Promise((resolve, reject) => {
        ...
        timerRef.current = setTimeout(async () => {
            try {
                let url = `/settings/area/check-code?code=${encodeURIComponent(code)}`;
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title level={4} style={{ margin: 0, fontWeight: 700 }} className="text-slate-800">
                                {isEditMode ? 'แก้ไขข้อมูลโลเคชั่น' : 'เพิ่มโลเคชั่นใหม่'}
                            </Title>
                            <Text className="text-slate-500 text-sm font-light">
                                {isEditMode ? 'Location Information Update' : 'Create New Location'}
                            </Text>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col md:flex-row h-auto md:h-[400px]">
                            ...
                            <div className="w-full md:w-[300px] bg-slate-50 p-6 border-r border-gray-100 flex-shrink-0">
                                <div className="text-center mb-8 mt-4">
                                    <div className="w-32 h-32 bg-white rounded-2xl border-2 border-dashed border-slate-200 mx-auto flex flex-col items-center justify-center text-slate-400 mb-4 shadow-sm group hover:border-emerald-300 transition-colors cursor-default">
                                        <PushpinOutlined style={{ fontSize: '48px' }} className="group-hover:text-emerald-500 transition-colors" />
                                        <span className="text-xs mt-3 font-medium tracking-wide uppercase text-slate-400 group-hover:text-emerald-500">Location Point</span>
                                    </div>
                                    <div className="text-slate-500 font-medium text-sm">ข้อมูลหลักโลเคชั่น</div>
                                </div>

                                <div className="space-y-4">
                                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                                        <Form.Item
                                            label={<span className="text-slate-600 font-medium">รหัสโลเคชั่น <span className="text-red-500">*</span></span>}
                                            name="G_CODE"
                                            rules={[{ required: true, message: 'ระบุรหัสโลเคชั่น' }, { validator: validateCode }]}
                                            hasFeedback
                                            validateStatus={checkingCode ? 'validating' : undefined}
                                            className="mb-0"
                                        >
                                            <Input prefix={<IdcardOutlined className="text-slate-400" />} placeholder="เช่น LOC-01" className="font-mono bg-slate-50" maxLength={20} />
                                        </Form.Item>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-white">
                                ...
                                <div className="mb-8">
                                    <div className="flex items-center gap-3 mb-5">
                                        <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg"><EnvironmentOutlined /></div>
                                    ...
                                    <Form.Item
                                        label="ชื่อโลเคชั่น (Location Name)"
                                        name="G_NAME"
                                        rules={[{ required: true, message: 'กรุณาระบุชื่อโลเคชั่น' }]}
                                    >
                                        <Input prefix={<TagOutlined className="text-slate-400" />} placeholder="ระบุชื่อสถานที่ / จุดที่ตั้ง" />
                                    </Form.Item>

                                    {/* Description / Hint */}
                                    <div className="bg-emerald-50/50 rounded-lg p-4 flex gap-3 items-start mt-6">
                                        <CompassOutlined className="text-emerald-500 mt-1" />
                                        <div className="text-xs text-emerald-700/80 leading-relaxed">
                                            ข้อมูลนี้ใช้สำหรับระบุพิกัดหรือจุดที่ตั้งภายในองค์กร ช่วยให้การอ้างอิงสถานที่ในเอกสารมีความถูกต้อง
                                        </div>
                                    </div>
                                </div>
                            </div>
