...
import { Modal, Form, Input, App, Button, ConfigProvider, Spin, Select, Typography } from 'antd';
import {
    IdcardOutlined, TagOutlined, PlusCircleOutlined, EditOutlined,
    SaveOutlined, ApartmentOutlined, DeleteOutlined, SolutionOutlined,
    ClusterOutlined, InfoCircleOutlined
...
function ModalForm({ open, record, onClose, onSuccess, onDelete }) {
    ...
    const validateCode = (_rule, value) => new Promise((resolve, reject) => {
        ...
        timerRef.current = setTimeout(async () => {
            try {
                let url = `/settings/position/check-code?code=${encodeURIComponent(code)}`;
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <Text className="text-slate-500 text-sm font-light">
                                {isEditMode ? 'Position Information Update' : 'Create New Position'}
                            </Text>
                        </div>
                    </div>
                            ...
                            <div className="w-full md:w-[300px] bg-slate-50 p-6 border-r border-gray-100 flex-shrink-0">
                                <div className="text-center mb-8 mt-4">
                                    <div className="w-32 h-32 bg-white rounded-2xl border-2 border-dashed border-slate-200 mx-auto flex flex-col items-center justify-center text-slate-400 mb-4 shadow-sm group hover:border-cyan-300 transition-colors cursor-default">
                                        <SolutionOutlined style={{ fontSize: '48px' }} className="group-hover:text-cyan-500 transition-colors" />
                                        <span className="text-xs mt-3 font-medium tracking-wide uppercase text-slate-400 group-hover:text-cyan-500">Job Position</span>
                                    </div>
                                    <div className="text-slate-500 font-medium text-sm">ข้อมูลหลักตำแหน่ง</div>
                                </div>

                                <div className="space-y-4">
                                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                                        <Form.Item
                                            label={<span className="text-slate-600 font-medium">รหัสตำแหน่ง <span className="text-red-500">*</span></span>}
                                            name="G_CODE"
                                            rules={[{ required: true, message: 'ระบุรหัสตำแหน่ง' }, { validator: validateCode }]}
                                            hasFeedback
                                            validateStatus={checkingCode ? 'validating' : undefined}
                                            className="mb-0"
                                        >
                                            <Input prefix={<IdcardOutlined className="text-slate-400" />} placeholder="เช่น POS-001" className="font-mono bg-slate-50" maxLength={20} />
                                        </Form.Item>
                                    </div>
                                </div>
                            </div>

                            {/* RIGHT SIDE: Details (Name & Department) */}
                            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-white">

                                {/* Section 1: General Info */}
                                <div className="mb-8">
                                    <div className="flex items-center gap-3 mb-5">
                                        <div className="p-2 bg-cyan-50 text-cyan-600 rounded-lg"><InfoCircleOutlined /></div>
                                    ...
                                    <Form.Item
                                        label="ชื่อตำแหน่ง (Position Name)"
                                        name="G_NAME"
                                        rules={[{ required: true, message: 'กรุณาระบุชื่อตำแหน่ง' }]}
                                    >
                                        <Input prefix={<TagOutlined className="text-slate-400" />} placeholder="ระบุชื่อตำแหน่ง (ภาษาไทย)" />
                                    </Form.Item>
                                </div>
                                ...
                                <div>
                                    ...
                                    <Form.Item
                                        label="อยู่ภายใต้แผนก (Department)"
                                        name="department_code"
                                        rules={[{ required: true, message: 'กรุณาเลือกแผนกต้นสังกัด' }]}
                                    >
                                        <Select
