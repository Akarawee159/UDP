...
function ModalForm({ open, record, onClose, onSuccess, onDelete }) {
    ...
    const validateCode = (_rule, value) => new Promise((resolve, reject) => {
        ...
        timerRef.current = setTimeout(async () => {
            try {
                const res = await api.get('/settings/zone/check-code', {
                    params: { code, excludeId: isEditMode ? record.G_ID : undefined }
                });
                setCheckingCode(false);
                if (res.data?.exists) {
                    reject(new Error('รหัสโซนนี้มีอยู่ในระบบแล้ว'));
                } else {
                    resolve();
                }
            } catch (err) {
                    ...
                    <div className="flex items-center gap-3 text-slate-800">
                        ...
                        <div>
                            <h3 className="text-lg font-bold m-0 leading-tight">
                                {isEditMode ? 'แก้ไขข้อมูลโซน' : 'เพิ่มข้อมูลโซน'}
                            </h3>
                            <span className="text-xs text-slate-700">
                                {isEditMode ? 'ปรับปรุงรายละเอียดโซนเดิม' : 'สร้างโซนใหม่ในระบบ'}
                            </span>
                        </div>
                    </div>
