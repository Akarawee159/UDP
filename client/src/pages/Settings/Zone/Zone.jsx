// src/pages/Masterdata/Zone/Zone.jsx
...
function Zone() {
    ...
    // ✅ Fetch API: /settings/zone
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const res = await api.get('/settings/zone');
    ...
    // ✅ Socket: zone:upsert, zone:delete
    useEffect(() => {
        const s = getSocket();
        if (!s) return;
        ...
        s.on('zone:upsert', onUpsert);
        s.on('zone:delete', onDelete);

        return () => {
            s.off('zone:upsert', onUpsert);
            s.off('zone:delete', onDelete);
        };
    }, []);
        ...
        >
            <div style={containerStyle} className="bg-gray-50">
                <div className="w-full mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4 flex-none">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">ข้อมูลโซน</h1>
                        <p className="text-slate-700 text-sm mt-1 pl-1">จัดการข้อมูลโซนในระบบ</p>
                    </div>
                    <div className="flex items-center gap-3 bg-white p-1.5 rounded-xl shadow-sm border border-gray-100">
                        <Input prefix={<SearchOutlined className="text-gray-400" />} placeholder="ค้นหารหัส, ชื่อโซน..." allowClear bordered={false} onChange={(e) => setSearchTerm(e.target.value)} className="w-full md:w-64 bg-transparent" />
                        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate} className="bg-blue-600 hover:bg-blue-500 border-none h-9 rounded-lg px-4 font-medium shadow-md">เพิ่มโซน</Button>
                    </div>
                </div>
