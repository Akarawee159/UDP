...
function ModalForm({ open, record, onClose, onSuccess }) {
    ...
    const fetchDetail = useCallback(async (id) => {
        try {
            setFetching(true);
            const res = await api.get(`/settings/countingunit/${id}`);
    ...
    const handleOk = async () => {
        try {
            ...
            let resData;
            if (isEditMode) {
                const res = await api.put(`/settings/countingunit/${record.G_ID}`, payload);
                message.success('อัปเดตข้อมูลสำเร็จ');
                resData = res?.data?.data;
            } else {
                const res = await api.post('/settings/countingunit', payload);
                    ...
                    <div className="p-8">
                        <Form form={form} layout="vertical" autoComplete="off" className="space-y-2">
                            ...
                            <Form.Item label={<span className="font-semibold text-gray-700">ชื่อหน่วยนับ</span>} name="G_NAME" rules={[{ required: true, message: 'กรุณาระบุชื่อหน่วยนับ' }]}>
                                <Input prefix={<TagOutlined className="text-gray-400" />} placeholder="ระบุชื่อหน่วยนับ" className="h-11 rounded-lg border-gray-200 focus:border-blue-500 hover:border-blue-400 bg-gray-50 focus:bg-white transition-all" allowClear />
                            </Form.Item>
                        </Form>
                    </div>
                </Spin>
