...
function SystemInList({ open, onCancel, targetDraftId }) {
    ...
    // Status Logic
    const [bookingStatus, setBookingStatus] = useState('30');
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (currentDraftId) {
                ...
                await api.post('/smartpackage/systemin/init-booking', { draft_id: newId, type: 'good' });
    ...
    const handleGenerateRef = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                message.success('สร้างเลขรับเข้าของดีเรียบร้อย');
            }
        } catch (err) {
            message.error('สร้างเลขรับเข้าของดีไม่สำเร็จ');
        }
    };
    ...
    const handleSaveHeader = async () => {
        ...
        } catch (err) { message.error('กรุณาระบุข้อมูลให้ครบถ้วน'); }
    };
    ...
    const handleScanProcess = async (qrString) => {
        ...
        if (!refID) {
            modal.warning({ title: 'แจ้งเตือน', content: 'กรุณาสร้างเลขรับเข้าของดีก่อนทำการสแกน', okText: 'รับทราบ', onOk: () => processingRef.current = false });
            return;
        }
        if (bookingStatus === '30') {
            modal.warning({
        ...
        try {
            ...
            if (res.data.success) {
                ...
                if (code === 'ALREADY_SCANNED') {
                    ...
                    modal.warning({
                        title: 'แจ้งเตือน',
                        content: msg, // ข้อความจาก Backend: "ทรัพย์ต้องมีสถานะจ่ายออกแล้วเท่านั้น..."
                        okText: 'รับทราบ',
                        onOk: () => { processingRef.current = false; },
                        afterClose: () => { processingRef.current = false; }
                    });
    ...
    const isEditingDisabled = !refID || bookingStatus === '33';
    const hasScannedItems = scannedList.length > 0;
    const showSaveCancel = refID && bookingStatus !== '33' && bookingStatus !== '32' && !hasScannedItems;
    const showConfirm = (bookingStatus === '31' || bookingStatus === '32') && hasScannedItems;
    const showCancelButton = bookingStatus !== '33' && !hasScannedItems;
    ...
    return (
        <ConfigProvider theme={{ token: { colorPrimary: '#16a34a', borderRadius: 6 } }}>
            <Modal
                title={<Title level={4} style={{ margin: 0 }}>{targetDraftId ? 'แก้ไขรายการรับเข้าของดี' : 'สร้างรายการรับเข้าของดี'}</Title>}
                open={open}
                onCancel={onCancel}
                width="95%"
                style={{ top: 20 }}
                        ...
                        <Col xs={24} md={7}>
                            <Card title="ข้อมูลจ่ายออก" className="h-full shadow-sm" size="small">
                                <Form layout="vertical" form={form}>
                                    ...
                                    {/* ✅ ปรับปรุงส่วน Input */}
                                    <Form.Item label="เลขรับเข้าของดี" name="refID">
                                        <Input
                                            placeholder="กดปุ่มเพื่อสร้าง"
                                            readOnly
                                            className={refID ? "bg-green-50 text-green-700 font-bold border-green-200" : ""}
                                            style={refID ? { backgroundColor: '#f8fffa', color: '#12873d' } : {}}
                                            addonAfter={
                                                <Button
                                                    type="primary"
                                                    size="small"
                                        ...
                                        {showConfirm && (
                                            <Col span={24} className="mt-2">
                                                <Button type="primary" block icon={<CheckCircleOutlined />} onClick={handleFinalize} size="large" className="bg-green-600 hover:bg-green-500">
                                                    รับเข้าของดี (Confirm)
                                                </Button>
                                            </Col>
                                        )}

                                        {bookingStatus === '33' && canUse('system-out:unlock') && (
                                            <Col span={24}>
